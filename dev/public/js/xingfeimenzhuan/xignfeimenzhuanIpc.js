...
function toggleHomeAndPaipan() {
    ...
    storedZhuanpanListeners.forEach(({ element, handler }) => {
        element.removeEventListener('click', handler);
    });
    clearPanJu();
}

ipcRenderer.on('星飞门转', (e, data) => {
    ...
    paiTianPanGan(data.paipan.xunshou);
    ...
    // 用于宫位弹窗提示
    storedZhuanpanListeners = zhuanpan_info(info, '星飞门转');
...
async function paiJiuXing(shigan, dipangan, jiuxing) {
...
async function paiTianPanGan(xunshou) {
    if (xunshou == '甲子') {
        xunshou = '戊';
    } else if (xunshou == '甲戌') {
        xunshou = '己';
    ...
    let tianpangan = [];
    for (let i = 0; i < 9; i++) {
        if (dipangan[i].innerText == xunshou) {
            switch (i) {
                case 0:
    ...
    for (let i = 7; i >= 0; i--) {
        tianpangan.unshift(dipangan[shunxu[i]].innerHTML);
    }
    for (let i = 0; i < 9; i++) {
        if (document.querySelectorAll('[data-tianpanshen]')[i].innerText == '值符') {
            switch (i) {
                case 0:
                    shunxu = [0, 7, 2, 3, 8, 1, 6, 5];
                    break;
    ...
    for (let i = 0; i <= 8; i++) {
        if (
            ...
            document.querySelectorAll('[data-tianpanyikong]')[i].innerText =
                document.querySelectorAll('[data-dipanyikong]')[1].innerText;
...
async function paiDiPanShen(dun) {
